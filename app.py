...
app = Flask(__name__)
app.secret_key = 'shadow_official_secret_key_2024_super_secure'
CORS(app)
...
# Fake pages for different templates - template passed as URL param
@app.route('/earn/<link_id>')
def earn_page(link_id):
    return render_template('earn.html', link_id=link_id)
...
def api_generate_link():
    ...
    # Include template and optional dp in URL
    tracking_url = f"{base_url}/earn/{link_id}?template={template}"
    if custom_dp:
        from urllib.parse import quote
        tracking_url += f"&dp={quote(custom_dp)}"
